page 50240 "E3 Get Groupping Indent Lines"
{
    Caption = 'Get Groupping Purchase Stock Indent Lines';
    PageType = List;
    ApplicationArea = All;
    ...
    DeleteAllowed = false;
    ModifyAllowed = false;
    SourceTableView = sorting("Document No.", "Line No.") order(descending) where("Released Stock Issue Purchase" = const(true), Remarks = filter('PO Qty' | 'Free Qty'));
    ...
    procedure SetPurchaseOrder(
        DocumentType: Enum "Purchase Document Type";
        DocumentNo: Code[20])
    begin
        PurchaseDocumentType := DocumentType;
        PurchaseDocumentNo := DocumentNo;
    end;

    procedure GetPurchaseOrder(
        var DocumentType: Enum "Purchase Document Type";
