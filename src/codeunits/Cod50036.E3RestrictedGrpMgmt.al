codeunit 50036 "E3 Restricted Group Mgmt."
{
    ...
    procedure SendRestrictedGroupDetails(var RestrictedGroupUpdateLog: Record "E3 Restricted Group Master"): Boolean
    var
    ...
    begin
        ...
        if not E3APISetup."Restricted Group API Enabled" then
            exit(false);

        E3APISetup.TestField("Restricted Group API");
        ...
        Clear(RootObj);
        RootObj.Add('d365_restrictedGrpMastMast', ItemArray);
        ...
        RequestMessage.SetRequestUri(E3APISetup."Restricted Group API");
        ...
        RestrictedGroupUpdateLog.Response :=
            CopyStr(JsonResponse, 1, MaxStrLen(RestrictedGroupUpdateLog.Response));
        ...
        if ResponseMessage.IsSuccessStatusCode then begin
            ...
            if ResponseRoot.SelectToken('d365_restrictedGrpMastStatus', ResponseToken) then begin
                ...
                for J := 0 to ResponseArray.Count - 1 do begin
                    ...
                    if ResponseMsg = 'Created Successfully' then begin
                        RestrictedGroupUpdateLog.IsSent := true;
                        RestrictedGroupUpdateLog.Response :=
                            CopyStr(ResponseMsg, 1, MaxStrLen(RestrictedGroupUpdateLog.Response));
                        RestrictedGroupUpdateLog."Last Sent" := CurrentDateTime;
                        RestrictedGroupUpdateLog.Modify(true);
                        exit(true);
                    end;
                end;
            end;

            RestrictedGroupUpdateLog.IsSent := false;
            RestrictedGroupUpdateLog.Response :=
                CopyStr(JsonResponse, 1, MaxStrLen(RestrictedGroupUpdateLog.Response));
            RestrictedGroupUpdateLog."Last Sent" := CurrentDateTime;
            RestrictedGroupUpdateLog.Modify(true);
